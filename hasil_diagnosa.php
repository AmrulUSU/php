...
<a style="background-color:#CCC;" href="index.php?top=peternak_add_fm.php"><strong>BACK HOME</strong></a></div></td>
  </tr>
  <tr bgcolor="#ffffff">
  ...
  <tr>
    ...
    <?php
    include "koneksi.php";
	$query_peternak=mysql_query("SELECT * FROM peternak ORDER BY id_peternak DESC");
	$data_peternak=mysql_fetch_array($query_peternak);
	echo "Nama : <br>". $data_peternak['nama'] . "<br>";
	echo "<hr>Jenis Kelamin :<br> ";
	$lk=$data_peternak['kelamin']; if($lk=="L"){ echo "Laki-laki"; }else { echo "Perempuan";}; echo "<br>";
	echo "<hr>Alamat : <br>". $data_peternak['alamat']. "<br>";
	echo "<hr>Email : <br>". $data_peternak['email']. "<br>";
	echo "<hr>";
	?></td>
    <td width="73%">    <?php
    include "koneksi.php";
  ...
  <tr bgcolor="#ffffff">
	...
	while($dataRelasi=mysql_fetch_array($query_relasi)){
		...
		echo "<p>Cari Data Gejala dan Bobot di Kasus Lama Pada Jenis Penyakit $dataRelasi[kd_penyakit]</p>";
  ...
  <tr bgcolor="#FFFFFF">
    <td align="center"><div style='border-radius:50px 50px;background-color:#0099FF; text-align:center; padding:2px 2px 2px 5px; color:#ffffff;'><strong>HASIL DIAGNOSA PENYAKIT AYAM</strong></div>
<p style="color:#06F; font-weight:bold;">BERDASARKAN HASIL DIAGNOSA PENYAKIT AYAM MAKA DIPEROLEH HASIL YANG TERDETEKSI PENYAKIT ADALAH :</p>
<?php
$query_temp=mysql_query("SELECT * FROM peternak ORDER BY id_peternak DESC") or die(mysql_error());
			$row_peternak=mysql_fetch_array($query_temp)or die(mysql_error());
			$id_peternak=$row_peternak['id_peternak'];
			$nama=$row_peternak['nama'];
			$kelamin=$row_peternak['kelamin'];
			$alamat=$row_peternak['alamat'];
			$email=$row_peternak['email'];
			$tanggal =$row_peternak['tanggal'];
arsort($arrNtotal);
...
foreach($arrNtotal as $kdK=>$Nilai){
	...
	$query_hasil="INSERT INTO analisa_hasil(id_peternak,kd_penyakit,persentase,tanggal) VALUES ('$id_peternak','$kd_penyakit','$persen','$tanggal')";
...
?>
  ...
  <tr bgcolor="#FFFFFF">
    <td><a href="proses-diagnosa.php?top=konsultasifm.php"><strong>&laquo;&laquo;Kembali</strong></a>
    </td>
  </tr>
</table>
</div>
